...
독일어 원문은 전체 2,974개 레코드에 들어 있다. 한국어는 별도 캐시에서 관리하며 현재 대표 구절 14개의 직접 번역 초안이 들어 있다. 빈 `korean` 필드는 번역 대기 상태라는 뜻이다. 현대 한국어 출판 번역은 사용하지 않는다.
...
`npm run check`는 원문을 다시 파싱하고 세 JSON을 만든 뒤, 2,000개 이상 구절·ID 유일성·JGB/GM 전체 절 범위·구절 길이·출전 URL을 검증한다. 원문 스냅샷을 다시 받을 때만 `python scripts/fetch_sources.py`를 사용한다. 빌드는 vendored 원문으로 재현되므로 평소에는 네트워크가 필요 없다.
