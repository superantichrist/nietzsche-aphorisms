...
조각은 편집자가 고른 선집이 아니라 전체 코퍼스를 나눈 전송·캐시 단위다. 번역 여부와 무관하게 모든 본문 구절이 선택 대상이며, 한국어가 아직 없으면 해당 독일어 또는 라틴어 원문을 표시한다. 스크립트 상단의 `WORK_FILTER`는 `all`, `nietzsche`, `schopenhauer`, `seneca` 또는 개별 작품 키로 바꿀 수 있다.
