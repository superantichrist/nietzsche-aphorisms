...
현재 26,783개 가운데 25,141개에 직접 만든 한국어 번역이 있으며 그중 5,749개는 감수를 마쳤다. 《인생의 짧음에 대하여》는 제1–20장 전체, 《도덕 서한》은 제1–60서한까지 1차 번역을 마쳤고 나머지 서한은 라틴어 원문을 먼저 공개한 뒤 순차 번역한다. 한국어는 원문과 분리된 캐시에서 관리한다. 번역이 붙은 레코드는 가능한 한 ID를 유지하되, 원문 누락이나 잘못된 순서·문단 및 시행 묶음이 발견되면 ID 보존보다 원문 완전성과 자연스러운 읽기 단위를 우선한다.
