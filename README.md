...
현재 24,298개 모두에 직접 만든 한국어 1차 번역이 있고 그중 5,749개는 감수를 마쳤다. 한국어는 독일어 원문과 분리된 캐시에서 관리한다. 번역이 붙은 레코드는 가능한 한 ID를 유지하되, 원문 누락이나 잘못된 순서·문단 및 시행 묶음이 발견되면 ID 보존보다 원문 완전성과 자연스러운 읽기 단위를 우선한다.
