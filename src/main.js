...
const elements = Object.fromEntries(
  [
    "quote-card", "quote-number", "translation-badge", "quote-korean", "quote-german", "original-language-label",
...
function originalLanguageLabel(quote) {
  return quote?.originalLanguage === "la" ? "LATINE · ORIGINAL" : "DEUTSCH · ORIGINAL";
}

function originalLanguageKo(quote) {
  return quote?.originalLanguage === "la" ? "라틴어" : "독일어";
}
...
function locationLabel(quote) {
  ...
  if (quote.sourceSection) {
    ...
    const sourceParagraphLabel = quote.sourceSectionParagraphCount > 1
      ? ` · 문단 ${sourceParagraph + 1}`
      : "";
    return `${part}${section}${title} · §${quote.sourceSection}${sourceParagraphLabel} · 문장 ${sentenceIndex + 1}`;
  }
...
function showQuote(quote, { historyMode = "replace" } = {}) {
  ...
  const original = originalText(quote);
  ...
  elements["quote-korean"].textContent = translated ? quote.korean : original;
  elements["quote-korean"].classList.toggle("german-fallback", !translated);
  elements["quote-german"].textContent = original;
  elements["original-language-label"].textContent = originalLanguageLabel(quote);
  elements["source-work"].textContent = `${quote.authorNameKo} · ${quote.workTitleKo} · ${quote.workTitleOriginal || quote.workTitleDe}`;
...
async function copyCurrent(questionMode = false) {
  ...
  const language = originalLanguageKo(quote);
  const text = questionMode
    ? `다음 ${quote.authorNameKo} 구절을 ${language} 원문과 작품의 문맥을 기준으로 검토하고 설명해 주세요. 번역의 뉘앙스, 핵심 개념, 필요한 역사·문헌 배경도 구분해서 알려 주세요.\n\n${context}\n\n[추가 질문]\n`
    : context;
  try {
...
function sectionHeading(quote) {
  const base = quote.sectionLabel || (UNNUMBERED_SECTIONS.has(quote.section)
    ...
    : `§${quote.section}`);
  const title = quote.sectionTitleOriginal || quote.sectionTitleDe;
...
function runSearch(query) {
  ...
  const matches = state.quotes.filter((quote) =>
    `${originalText(quote)}\n${quote.korean}\n${quote.authorNameKo}\n${quote.authorNameOriginal || quote.authorNameDe}\n${quote.workTitleKo}`
  ...
  matches.slice(0, 50).forEach((quote) => {
    ...
    text.textContent = quote.korean || originalText(quote);
...
async function init() {
  ...
  try {
    ...
    elements["stat-author-seneca"].textContent = state.manifest.authors.seneca.count.toLocaleString("ko-KR");
