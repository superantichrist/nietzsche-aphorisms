...
현재 24,300개 중 23,600개에 직접 만든 한국어 번역이 있고 그중 5,749개는 감수를 마쳤다. 남은 쇼펜하우어 구절도 장 단위 문맥을 유지해 직접 번역하며 증분으로 추가한다. 한국어는 독일어 원문과 분리된 캐시에서 관리한다. 번역이 붙은 레코드는 가능한 한 ID를 유지하되, 아직 미번역인 700개 구간은 ID 보존보다 원문 완전성·올바른 순서·자연스러운 문단 및 시행 묶음을 우선한다.
